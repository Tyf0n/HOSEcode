...
public class HOSECodeGenerator12282013 implements java.io.Serializable
{

	private static ILoggingTool logger =
	    LoggingToolFactory.createLoggingTool(HOSECodeGenerator12282013.class);
...
//    private static final long serialVersionUID = -4353471818831864513L;
	...
	private int [] spherePosi;///store the positions of atoms
	...
	public HOSECodeGenerator12282013()
	{
	...
	private void ensureIsotopeFactory(IChemObjectBuilder builder) throws CDKException {
		if (isotopeFac == null) {
			try {
	            isotopeFac = IsotopeFactory.getInstance(DefaultChemObjectBuilder.getInstance());
            } catch (IOException e) {
   ...
   * for this method to work properly.
	...
	public String getHOSECode(IAtomContainer ac, IAtom root, int noOfSpheres, boolean ringsize) throws CDKException
	{
		...
		spherePosi[ac.getAtomNumber(root)] = -1;///
		root.setFlag(CDKConstants.VISITED, true);
		rootNode = new TreeNode(root.getSymbol(), null, root, (double)0, atomContainer.getConnectedBondsCount(root), 0);
		/*
		 *  All we need to observe is how the ranking of substituents
		...
		for (int i = 0; i < conAtoms.size(); i++){
			try{
				...
				if(atom.getSymbol().equals("H"))
					continue;
				spherePosi[atomContainer.getAtomNumber(atom)] = sphere;///
			...
			if (!("&;#:,".indexOf(treeNode.symbol) >= 0))
			{
				...
				if (conAtoms.size() == 1){
					...
					for (int j = 0; j < conAtoms.size(); j++)
					{
						...
						if(toNode.getSymbol().equals("H")) {continue;}///
						if (toNode != treeNode.source.atom)
						{
							bond = atomContainer.getBond(node, toNode);
							if (sphere - spherePosi[atomContainer.getAtomNumber(toNode)] <= 2 && sphere - spherePosi[atomContainer.getAtomNumber(toNode)] >= 0) {nextSphereNodes.add(new TreeNode("&", treeNode, toNode, bond.getOrder().numeric(), 0, treeNode.score));continue;}///
							if (bond.getFlag(CDKConstants.ISAROMATIC))
							{
							...
							spherePosi[atomContainer.getAtomNumber(toNode)] = sphere + 1;///
...
/*
	...
	private String getSphereCode(List<TreeNode> sphereNodes) throws CDKException
	{
		...
		TreeNode treeNode = null;
		StringBuffer code = new StringBuffer();code.append("_" + sphereNodes.size() + "_");///
		/*
		...
		for (int i = 0; i < sphereNodes.size(); i++)
		{
			...
			if (!treeNode.source.stopper && treeNode.source.atom == branch)
			{
				...
				{
					tempCode.append(atomContainer.getAtomNumber(treeNode.atom) + 1);///
					tempCode.append(treeNode.symbol);///
///					tempCode.append(getElementSymbol(treeNode.symbol));
	...
	private void sortNodesByScore(List<TreeNode> sphereNodes)
	{
		...
		do
		{
			...
			{
				if (!((TreeNode) sphereNodes.get(i)).source.equals((TreeNode) sphereNodes.get(i + 1).source)) {/*System.out.println("#");*/continue;}///
